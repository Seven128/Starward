...
- Keep required product meaning, confirmed technical choices and present implementation limits distinguishable at their existing owners. A newly discovered data/implementation limit does not revise the required capability by itself. When a supplier field or approximation changes what users can learn or do, resolve that difference before writing it as the current product contract; visual adoption does not implicitly approve its data semantics.
- Architecture entries describe the actual composition and its unresolved dependencies, not just a list of implemented modules. Reference the source/configuration/publication owners needed for the outcome; keep account values and live status external. Do not turn “adapter implemented”, “assets published” or a structural validation into “capability available” in an owner or adopted-resource entry. Remove or narrow an overstated summary when contradicted, while retaining valid design adoption and unrelated implementation facts.
...
2026-09-13 用户明确：项目设计资源持久化，每个范围长期只留当前被采用的一份。2026-09-23 补充：旧版本也属于废案清理范围；每次需求开发的设计资源候选完成时集中清理，删除前须向用户确认本轮清单并由执行者逐项核对。本节适用于项目各平台的设计资源及其本地/远端可编辑源，是项目 Skill 执行资源留存与清理的唯一规则 owner。此前“原稿永久保留”“保留全部修订/历史包”等描述不构成例外；通用工具的保存步骤只保障探索过程可恢复，不要求项目永久留存废案。
...
- **探索期临时并存。** 设计资源候选制作、比较和修复期间，原始导出与中间修订可以临时保留；不得充当当前采用依据。明确否定、已被同范围当前候选完全替代且不再提供有效依赖的旧版本也属于废案。新稿仍待采用时保持旧采用源，不把“更新了产品要求”“生成成功”或“检查通过”当成视觉采用，也不为清理而自动采用最新稿或删除待决输入；交付中如实说明尚未完成定稿收敛。
- **候选完成时集中清理。** 每一次需求开发的设计资源候选完成、可供审阅时，集中核对本次范围内未采用、已否定、被完全替代的候选包和旧版本、中间原稿/修订、过期预览与截图、重复导出及仅服务于它们的对比/生成记录。删除前逐项核验对象、当前采用源、待审输入和实际依赖，向用户说明具体删除清单并**每次向用户确认一次**；得到确认后执行并读回结果，不凭旧日期/目录名批量删除。不把废案移到项目中的history/archive、另一任务目录或压缩包继续永久保存。必要的拒绝原因和选择边界以简短文字留在原Context owner；历史恢复使用已有Git历史或回收站，不制造新的历史素材归档。
...
- **Stitch与本地同步收敛。** Stitch设计项目/画布是持久化设计资源的一部分，也是本规则的重点清理对象，不能只整理本地Context或导出目录。每次设计资源候选完成并获得本轮删除确认后，核对Stitch中当前页面/组件/必要状态变体与本地当前包，再删除本次范围内未采用、已否定和完全被替代的screen/画板/旧修订，以及仅包含这些废案的重复项目。共享Stitch项目只删除废弃节点，不删除其他仍有效或待决的设计；不以隐藏、改名“废案”、移到归档项目代替删除。同步更新项目/节点链接、本地导出与预览、采用入口和Context引用，并读回远端剩余画板、打开本地当前资源核对。远端未清理或采用源未对齐时，不宣称两端已完成收敛；Figma等其他持久化设计工具同样处理。
