...
This Context owns durable responsibility and main-versus-drilldown placement for the Starward native mobile product, the independently scoped WeChat Mini Program, and owner operations surfaces. It does not own visual token values, authored target pixels, delivery scope, implementation status, or one-off evidence. The Mini Program surfaces below implement the current accepted product/technical Source under its correction/disposition precedence and the adopted Field Signal system target; document version labels remain provenance only and exact visual values remain owned only by `DESIGN.md`. Owner instruction on 2026-09-02 retires `target-miniapp-sky-canvas-current-constraint` as current Source, so the Mini Program consumes its owning Screen Contract directly and treats that resource as historical audit evidence only. `target-operations-sky-canvas-current-constraint` remains ordinary Source for its separately scoped owner-operations surface; neither record proves production conformance.
...
- The WeChat Mini Program is one independently completable current product carrier, evolved in place under the current accepted Source. It coexists with the native App and does not prove, replace, inherit, or silently shrink that product. Its primary navigation is exactly Map and My; formal-spot basic and astronomy information now belong to one Map-parallel information panel, while full-sky orientation remains its only astronomy child route.
...
## Product Surface Detail Routing

The registered nodes below continue this same cross-surface contract; they do not create new products or visual authorities. Read the affected platform node together with the cross-surface rules in this root.

- [Mobile discovery, observing, map, spot, itinerary and collaboration responsibilities](product-surfaces/mobile-core.md)
- [Mobile orientation, shooting, offline safety, contribution, notifications and identity responsibilities](product-surfaces/mobile-field-and-account.md)
- [WeChat Mini Program Map/Search/spot panel, full-sky, My, contribution and profile responsibilities](product-surfaces/wechat-miniapp.md)
- [Owner data operations and release-quality responsibilities](product-surfaces/operations.md)
...
- Native App main surfaces show decision, next action, and decision-relevant uncertainty. Mini Program surfaces show relevant objective facts, user plans and explicit uncertainty rather than a combined departure conclusion. Professional evidence, diagnostics, raw provider material, audit history, and operational controls stay in their owning drilldown or operations surface.
...
- The native App CNY 200/month ceiling and paid-default-zero policy are visible at decision points that could consume external cost. Budget state may degrade or block a capability; it never silently purchases, upgrades, or adds a paid source.
- The Mini Program independently uses a current external-services hard ceiling of CNY 350/calendar month excluding application/database IaaS. This 2026-09-09 owner decision replaces earlier Mini Program 200/300 ceilings; budgets are not added together. It is a spending limit for the current personal non-commercial trial, not proof of capability, reachability or sufficient quota. That budget does not authorize purchase, commercial use, provider promotion or public production traffic; provider/license/cost gates remain explicit.
