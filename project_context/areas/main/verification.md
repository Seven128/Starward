...
- WeChat Mini Program environment/owned paths, isolation boundaries, selected-visual-target routing, and deterministic → H5 → DevTools → representative-device evidence promotion live in the root-owned on-demand `project_context/development-workflow.md`; its Product/Screen Contract lives in `screen-contracts/wechat-miniapp.md`. Neither Context nor a command's existence is an acceptance result.
