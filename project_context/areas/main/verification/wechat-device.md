...
- For initial physical development feedback, use the official DevTools GUI QR-based true-debug flow on an authorized phone, following the preparation and transport diagnosis in the development-feedback child. USB/ADB is optional and becomes relevant when machine capture/input is needed. Official Android Platform Tools ADB observes the actual display and injects deliberate taps/swipes; the installed official `miniprogram-automator` SDK provides a separate, capability-checked runtime observation channel when supported. Verify each channel independently: GUI debugging does not require ADB authorization, and a remote connection does not establish successful SDK runtime reads.
