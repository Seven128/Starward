...
- Boundary: this target does not itself define destinations, page responsibilities, information hierarchy, account/auth, API/service reuse, framework, package architecture, route ownership, runtime behavior or acceptance journeys; those now come from V2.0 Source and owning Context. It remains visual authority only and cannot prove implementation.
...
- Only after the deterministic and full browser checks for that candidate are green and no relevant input is stale, cleanly regenerate `apps/wechat-miniapp/dist/weapp/**` and open the repository-native `apps/wechat-miniapp/project.config.json` through one exclusive formal invocation. The installed DevTools can open a copied temporary project, but its automation handshake omits `SDKVersion`; that alternative is therefore not a valid evidence environment. The formal runner fingerprints the complete generated bundle and declared production/tool inputs before use, owns a dynamic API port and the automator's SDK-standard loopback port `9420`, and fails startup on an existing listener. Its Windows launcher preserves the installed `cli.bat` bootstrap semantics by starting the executable in the DevTools installation directory and carrying the project path through the dedicated `cwd` environment binding. Automator log activation is explicitly awaited because the SDK's convenience console subscription emits a fire-and-forget request under Node 24; any remaining unhandled rejection is hash-recorded, forces the run to fail, and does not bypass teardown. A setup session first moves to the production, network-neutral auth page, replaces and exactly reads back only the app-owned persisted-state key with the canonical empty DAY/MY seed, then proves the production Settings DAY control and live `theme-day` class. It records request diagnostics, waits for runtime-event quiescence, detaches observation, and closes; its post-control state is deliberately discarded. A freshly opened evidence session again moves to auth and performs the sole evidence-state canonical reset/readback before diagnostics are cleared and journeys begin. Degradation starts with a healthy BFF and opens the named fault window only after that evidence reset, so an uncontrolled cold-start request can never be counted as an accepted fault. Removing storage alone is forbidden because a loaded store may repersist its previous mode. Runtime observers end only after quiescence; teardown and stable port closure are then proven separately. The same top-level invocation owns both isolated session stages, declared journeys, screenshots, candidate and bundle after-fingerprints, and process/port teardown; final evidence is written only after teardown has been observed. A pre-existing `wechat-devtools-session.json`, screenshot set or manually edited generated project is historical input and cannot be promoted to current evidence. Collect all tool-side findings, return to the fast loop, batch repairs, rerun affected cheap checks and the complete applicable browser journey, and rebuild/import weapp only after the repaired candidate is settled.
...
2. Freeze one candidate identity covering all acceptance-relevant Taro source, dependency lock, project/build/base-library configuration, fixture/data contract, selected assets, API contracts, acceptance scenarios, verifier/launcher closure and tool environment. The runner computes this identity before collection and rechecks it before emitting evidence; a changed input or incomplete closure fails stale rather than inheriting a previous pass.
...
| `apps/wechat-miniapp/project.private.config.json` | Implemented machine-local WeChat project configuration used by manual and formal DevTools sessions. | Exact-ignore, machine-local only. It must never be committed, copied into reports, or treated as shared configuration authority. |
...
| `tools/miniapp/**` | Implemented platform-runner/tool root, including clean H5 serving, deterministic icon generation/checking and current-session DevTools collection. | Trackable orchestration, health, fingerprint, DevTools import, process and scoped-cleanup code. Only implemented commands registered in package scripts count as capabilities. |
...
| `apps/wechat-miniapp/project.config.json` plus `dist/weapp/**` | Implemented canonical DevTools acceptance root and clean generated bundle. | A formal runner exclusively owns the DevTools session and weapp output, binds before/after candidate and bundle fingerprints, and fails on mutation or an unknown listener. |
...
- The local WeChat DevTools CLI is `C:\Program Files (x86)\Tencent\微信web开发者工具\cli.bat`; its current version/login/base-library/project identity must be captured by the runner, not hard-coded as a passing historical fact.
...
1. Define and update the owning product/surface Context for the selected Mini Program responsibilities, then consume `target.system.wechat-miniapp-soft-instruments-2026-08-05` without borrowing App/Admin design artifacts or inventing another visual authority.
