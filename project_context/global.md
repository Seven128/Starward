...
- `DESIGN.md` owns two independent active visual profiles: native App target `target.system.starward-blue-skeuomorphic-2026-07-29` and WeChat Mini Program target `target.system.wechat-miniapp-sky-canvas-field-signal-2026-09-02`. The Mini Program target defines visual-system values; its Product Surface and runtime responsibility are separately owned by the registered Mini Program Screen Contract and production code. Its immutable reviewed source package lives under `docs/design-resources/miniapp-design-system-2026-09-02-sky-canvas-field-signal/**`. The superseded `target.system.wechat-miniapp-sky-canvas-2026-08-25` is inactive rollback/reference material and cannot control current styling.
  The current page/interaction resources remain formal implementation constraints: `target-miniapp-sky-canvas-current-constraint` and `target-operations-sky-canvas-current-constraint`, with canonical records in their owning Screen Contracts and immutable Source/handoffs under `docs/design-resources/miniapp-design-system-2026-08-25-sky-canvas/**`. The Mini Program constraint keeps its composition and interaction meaning while conflicting visual values yield to the current system; the owner-operations constraint remains independently scoped outside that system. Selection and preflight prove no production, native, runtime or pixel-exact conformance.
