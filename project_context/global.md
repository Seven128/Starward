...
- The native App's selected design direction combines a blue visual system with disciplined skeuomorphism for professional observing and outdoor equipment. The independently selected WeChat Mini Program direction is `target.system.wechat-miniapp-sky-canvas-2026-08-25`; it makes map, sky and aligned time evidence the visual subject and does not inherit App values or targets.
...
- `DESIGN.md` owns two independent active visual profiles: native App target `target.system.starward-blue-skeuomorphic-2026-07-29` and WeChat Mini Program target `target.system.wechat-miniapp-sky-canvas-2026-08-25`. The Mini Program target defines visual-system values; its Product Surface and runtime responsibility are separately owned by the registered Mini Program Screen Contract and production code. The current Sky Canvas page resource is explicitly selected as two formal implementation constraints, `target-miniapp-sky-canvas-current-constraint` and `target-operations-sky-canvas-current-constraint`, with canonical records in their owning Screen Contracts and immutable Source/handoffs under `docs/design-resources/miniapp-design-system-2026-08-25-sky-canvas/**`. Earlier selected Mini Program handoff/resource closure and page/control targets persist only as named immutable-protocol exceptions with inactive visual styling, geometry and fidelity constraints; ordinary superseded candidates are deleted under the current-truth retention rule. Constraint selection and preflight prove no production, native or pixel-exact behavior.
