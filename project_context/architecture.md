...
- Mini Program generic UI substrate: the next implementation pass prefers pinned, per-component imports from MIT `@taroify/core` behind shared adapters under `apps/wechat-miniapp/src/components/**`, with one `ConfigProvider`/CSS-variable projection from the Mini Program DESIGN profile. Taroify may own generic field, form, list, popup, selection, uploader, progress, tabbar and candidate floating-panel mechanics; it cannot own Product Surface/Control semantics, persisted state, upload transport/idempotency, the Map presentation coordinator, exact tokens or iconography. `apps/wechat-miniapp/src/components/semantic-asset.tsx` remains the implementation icon/asset extension point; `@taroify/icons` is excluded. Taro enhanced `ScrollView` remains the curved-time-ruler scrolling owner. Package/version, tree shaking, bundle delta and WEAPP compatibility are implementation-time evidence, not established by this design decision.
