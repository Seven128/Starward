...
- Current-truth retention rule: keep one active representation of each responsibility. Remove superseded ordinary artifacts after consumers migrate; Git history is recovery. Preserve selected design sources and resources still read by project checks, plus any genuinely required audit evidence. Retained historical inputs cannot override current DESIGN/Context. Design iteration updates the current resource unless a project consumer requires immutable source identity; retirement of DSA/DRA alone never cancels adopted decisions.
