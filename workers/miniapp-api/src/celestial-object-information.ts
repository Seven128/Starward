...
import { hipparcosCatalogSources } from "./sky-scene-catalog-provider.ts";
...
const INTRODUCTIONS: Readonly<Record<string, string>> = Object.freeze({
  "HIP:32349": "天狼星是大犬座 α 星，也是 IAU 采用的正式恒星名称。它在当前亮星目录中拥有最低的 V 波段视星等。",
  "HIP:91262": "织女星是天琴座 α 星，IAU 正式名称为 Vega。它是北半球夏季夜空中醒目的亮星之一。",
  "HIP:11767": "北极星是小熊座 α 星，IAU 正式名称为 Polaris。它靠近北天极，常用于辨认北方。",
...
export class CelestialObjectInformationService {
  ...
  get(reference: string, locale = "zh-CN") {
    ...
    const row = hipparcosRowByReference(reference);
    if (!row) throw new Error("celestial_object_not_found");
    const catalog = loadHipparcosBrightStarCatalog();
    const sources = hipparcosCatalogSources(catalog);
    const introduction = INTRODUCTIONS[reference] ?? null;
    const aliases = [row.properName, `HIP ${row.hip}`, row.hd ? `HD ${row.hd}` : null]
    ...
    const data: CelestialObjectInformation = {
      reference,
      kind: "STAR",
      displayName: row.properName ?? `HIP ${row.hip}`,
      catalogId: `HIP ${row.hip}`,
      aliases,
      introduction,
      facts,
      contentState: introduction ? "READY" : "BASIC_ONLY",
