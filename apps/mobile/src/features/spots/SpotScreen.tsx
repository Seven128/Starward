...
const actions: Array<{ key: ViewKey; id: string; label: string; stableControlId: string; accessibilityName: string }> = [
  { key: "decision", id: "spot-open-detail", label: "决策摘要", stableControlId: "evidence-section-nav", accessibilityName: "证据分区导航" },
  { key: "light", id: "spot-open-light-details", label: "暗度与遮挡", stableControlId: "horizon-polar-view", accessibilityName: "地平线极坐标" },
  { key: "media", id: "spot-open-media", label: "媒体", stableControlId: "spot-media-gallery", accessibilityName: "地点媒体画廊" },
  { key: "access", id: "spot-open-access", label: "到达设施", stableControlId: "access-facility-fact-list", accessibilityName: "到达与设施事实" },
  { key: "safety", id: "spot-open-safety", label: "安全", stableControlId: "safety-block", accessibilityName: "安全阻断" },
  { key: "coordinate", id: "spot-share-coordinate", label: "坐标策略", stableControlId: "spot-action-dock", accessibilityName: "地点行动 Dock" },
  { key: "reviews", id: "spot-open-reviews", label: "评价可信度", stableControlId: "trust-panel", accessibilityName: "可信度面板" },
];
...
export function SpotScreen() {
  ...
  return <SafeAreaView testID="screen-spot-detail-and-trust" style={styles.screen}><ScrollView onScroll={restoration.onScroll} scrollEventThrottle={restoration.scrollEventThrottle} contentContainerStyle={styles.content}>
    ...
    {active === "reviews" ? <View style={styles.panel}><Evidence testID="spot-review-dimensions" title="多维评价暂无合格样本" body="暗度、开阔、云量、交通、设施、安全、拥挤、信号和摄影/目视适配将分别统计。" /><Evidence testID="spot-review-trust" title={`可信来源 ${detail?.trust.verifiedSources ?? 0} · 需现场核验`} body="贡献者等级、样本、时间和审核状态逐项展示，不压成单一星级。" /><Evidence testID="spot-review-moderation" title={`冲突：${detail?.trust.conflicts.join("、") || "暂无已识别冲突"}`} body="普通贡献不能覆盖官方或管理员安全阻断；可提交纠错并查看审核状态。" /><Pressable testID="spot-open-contribution" accessibilityRole="button" accessibilityLabel="从地点详情提交实况或纠错" onPress={() => router.push("/contribute")} style={styles.contribute}><Text style={styles.retryText}>提交实况、纠错或评价</Text></Pressable></View> : null}
...
const styles = StyleSheet.create({
  ...
  contribute: { minHeight: minimumTouchTarget, alignItems: "center", justifyContent: "center", borderRadius: radii.control, backgroundColor: palette.primaryActive },
});
