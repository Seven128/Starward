...
export class AdminOperationsService {
  ...
  command(command:AdminCommand){
    if(command==="provider-tco-evaluate"){const base={mau1k:cost(0),mau10k:cost(0),mau100k:cost(0),failure:cost(0)};const result=evaluateProviderCandidates({approvedPaidBudgetCny:2400,secondPaidSourceApproved:false,purchaseApproved:false,requiredLicenseGate:"personal-trial-license",candidates:[{id:"cheap-unqualified",capability:"weather",region:"CN",passedGates:["provenance"],priceEvidenceAt:this.now(),currency:"CNY",taxIncluded:true,scenarios:base},{id:"qualified-lowest-tco",capability:"weather",region:"CN",passedGates:["provenance","quality","stability","personal-trial-license","safe-degradation"],priceEvidenceAt:this.now(),currency:"CNY",taxIncluded:true,scenarios:{...base,mau10k:cost(680)}}]});this.state.provider={hardGates:"真实性、区域质量、稳定、个人非商业使用许可、安全降级",tco:"个人试用外部服务预算 CNY 200/月、2,400/年；合格免费优先，再比较同日含税完整 TCO",decision:`通过硬门槛后按完整总成本排序，候选为 ${result.decision.providerId}；预算内 ${result.decision.withinBudget ? "是" : "否"}，仍未授权购买或生产启用`};}
