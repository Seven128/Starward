...
export function ForecastCoverageNote({ starts, timezone, scopeKey, scope = "spot", stale = false, onExpandedChange }: { starts: readonly string[]; timezone: string; scopeKey: string; scope?: "spot" | "map" | "air"; stale?: boolean; onExpandedChange?: (open: boolean) => void }) {
  ...
  if (stale && ranges.length === 0) return null;
  ...
  return <View className="forecast-coverage">
    <View className="forecast-coverage__heading">
      <Text className="type-caption">{stale ? "上次获取的预报时段" : "可用预报时段"}</Text>
    ...
    {!ranges.length && !stale ? <StatusPanel state="EMPTY" emptyLevel="section" title="暂无可用预报时段"
      detail={scope === "map" ? "当前地图采样点未返回可用云量预报；移动地图或选择其他日期可重新查看。" : scope === "air" ? "本地点暂未返回空气质量小时预报；当前区域若有读数，会在上方单独显示。" : "本地点在所选时段未返回小时天气预报；天文资料仍可独立查看。"} /> : null}
