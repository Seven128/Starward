...
test("decode retry invalidates both cached representations before advancing the request revision", () => {
  ...
  const visit = (node: ts.Node) => {
    if (ts.isArrowFunction(node)) {
      ...
      if (body.includes("setCanvasDeepSkyImage(null)") && body.includes("setDeepSkyImageAsset(null)") && body.includes("setDeepSkyImageRetry")) handler = node;
    }
  ...
  assert.deepEqual(calls, ["canvas:null", "asset:null", "retry:5"]);
});
