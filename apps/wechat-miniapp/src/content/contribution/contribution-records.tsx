...
export function ContributionRecords({ form }: { form: ContributionForm }) {
  ...
  return <View className="contribution-records" data-control="contribution-records">
    ...
    {form.history.isPending ? <StatusPanel state="LOADING" detail="正在读取创建与反馈记录。" /> : form.history.isError ? <StatusPanel state="EMPTY" detail="暂时无法读取记录。" recoveryLabel="重试" onRecover={() => void form.history.refetch().catch(() => {})} /> : visible.length ? <View className="contribution-records__list">
