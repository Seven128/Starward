...
export function ContributionRecords({ form }: { form: ContributionForm }) {
  ...
  return <View className="contribution-records" data-control="contribution-records">
    ...
    {form.history.isPending ? <StatusPanel state="LOADING" detail="正在读取创建与反馈记录。" /> : form.history.isError ? <StatusPanel state="ERROR" detail="暂时无法读取记录。" recoveryLabel="重试" onRecover={() => void form.history.refetch().catch(() => {})} /> : visible.length ? <View className="contribution-records__list">
      {visible.map((item) => { const status = contributionRecordStatus(item); const submissionState = contributionSubmissionState(item).toLowerCase(); return <View className={`contribution-record contribution-record--${submissionState}`} key={item.submissionId}>
        ...
        {contributionSubmissionState(item) === "DRAFT" ? <SoftButton label={`继续编辑${recordName(item)}`} disabled={form.commandBusy} onClick={() => void Taro.navigateTo({ url: `/content/contribution/index?submissionId=${encodeURIComponent(item.submissionId)}` })}>继续编辑</SoftButton> : status.key === "REJECTED" ? <View className="contribution-record__actions"><SoftButton label={`查看${recordName(item)}审核意见`} disabled={form.commandBusy} onClick={() => setSelected(item)}>查看审核意见</SoftButton><SoftButton label={`修改并重新提交${recordName(item)}`} disabled={form.commandBusy} onClick={() => item.formalFeedback && item.spotId ? void Taro.navigateTo({ url: `/content/spot-feedback/index?spotId=${encodeURIComponent(item.spotId)}&spotName=${encodeURIComponent(recordName(item))}&submissionId=${encodeURIComponent(item.submissionId)}` }) : void Taro.navigateTo({ url: `/content/contribution/index?submissionId=${encodeURIComponent(item.submissionId)}` })}>修改并重新提交</SoftButton></View> : <SoftButton label={`查看${recordName(item)}本次记录`} disabled={form.commandBusy} onClick={() => setSelected(item)}>查看提交内容</SoftButton>}
      </View>; })}
