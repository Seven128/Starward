...
import { runtimeScript, providerScript, imageryScript, sanitizeRuntimeReport, sanitizeProviderSimulationReport } from "./runtime-diagnostics.mjs";
import { sanitizeImageryReport, imageryProbeSucceeded } from "./imagery-runtime-probe.mjs";
...
const failureCodes = new Set([...`${preflight}\n${runtimeScript}\n${imageryScript}`.matchAll(/(?:host_preflight|runtime_diagnostic)_[a-z0-9_]+/gu)].map(([code]) => code));

// Only these repository-owned fixed programs may run. Imagery invokes normal
// product reads (provider/cache/usage side effects); SQL probes remain read-only.
// No arbitrary command, remote file, credential export or deployment.
export function diagnoseHost(env, execute = spawnSync, temporaryRoot = tmpdir()) {
  ...
  if (!["host", "runtime", "providers", "imagery"].includes(mode)) return invalid;
  try {
  ...
  try {
    ...
    ], { input: mode === "imagery" ? imageryScript : mode === "providers" ? providerScript : mode === "runtime" ? runtimeScript : preflight, encoding: "utf8", timeout: 60_000, maxBuffer: 64 * 1024, env: childEnv, windowsHide: true });
    ...
    if (mode === "imagery") return sanitizeImageryReport(report);
