...
- 本次开发前工作完成时，按生命周期规则同步收敛Stitch画布与本地采用包：核对并迁出有效依赖，再实际删除未采用/被替代的Stitch画板及仅含废案的项目、本地废案及其过期预览/重复导出/专属记录，修复引用并读回两端结果。只改Context、只删本地或在Stitch隐藏/归档不算完成。待采用状态不得伪称前期已完成或自动采用最新稿。当前必要母版/分件和状态变体随采用包保留，Windows删除按AGENTS移入回收站。
