...
items.forEach((el,i)=>{el.setAttribute('role','button');el.tabIndex=0;const go=()=>setExtent('large',()=>{const target=document.querySelector(i?'#astronomy':'#overview');doc.scrollTo({top:Math.max(0,target.offsetTop-nav.offsetHeight-22),behavior:reduced()?'instant':'smooth'});target.tabIndex=-1;target.focus({preventScroll:true});activeNav(i);});el.addEventListener('click',go);el.addEventListener('keydown',e=>{if(['Enter',' '].includes(e.key)){e.preventDefault();go();}});});
