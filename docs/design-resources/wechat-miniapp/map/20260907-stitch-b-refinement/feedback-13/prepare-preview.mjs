...
console.log('Built feedback-13 companion (production unchanged).');
...
let polished=await fs.readFile(new URL('index.html',out),'utf8');
polished=polished.replace('</head>','<link rel="stylesheet" href="polish.css?v='+Date.now()+'"></head>').replace('</body>','<script src="polish.js?v='+Date.now()+'"></script></body>');
await fs.writeFile(new URL('index.html',out),polished);for(const f of ['polish.js','polish.css'])await fs.copyFile(new URL(f,root),new URL(f,out));

let scenes=await fs.readFile(new URL('index.html',out),'utf8');scenes=scenes.replace('</head>','<link rel="stylesheet" href="scenes.css"></head>').replace('</body>','<script src="scenes.js"></script></body>');await fs.writeFile(new URL('index.html',out),scenes);for(const f of ['scenes.js','scenes.css'])await fs.copyFile(new URL(f,root),new URL(f,out));

let meteorHtml=await fs.readFile(new URL('index.html',out),'utf8');meteorHtml=meteorHtml.replace('</head>','<link rel="stylesheet" href="meteor.css"></head>').replace('</body>','<script src="meteor.js?v=clockwise-clear-2"></script></body>');await fs.writeFile(new URL('index.html',out),meteorHtml);for(const f of ['meteor.js','meteor.css'])await fs.copyFile(new URL(f,root),new URL(f,out));
