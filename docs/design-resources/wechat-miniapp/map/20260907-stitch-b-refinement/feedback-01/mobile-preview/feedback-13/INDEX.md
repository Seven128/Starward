...
状态：待评审。入口：[评审页](review.html) · [手机中档](outputs/interactive/index.html?extent=medium) · [天文](outputs/interactive/index.html?extent=large&section=astronomy)。
...
可编辑变更：meteor.js、meteor.css、scenes.js；使用node prepare-preview.mjs重建（依赖前序基础壳），outputs/interactive可独立运行。唯一采用入口仍为../../ADOPTED.md，明确行为规则见所属Screen Contract，精确视觉待评审。
