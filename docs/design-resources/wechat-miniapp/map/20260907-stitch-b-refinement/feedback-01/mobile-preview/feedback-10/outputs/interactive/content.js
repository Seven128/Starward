...
function updateFacts(i){selectedSlice=i;const row=fixtureRows[i],partial=scenario==='partial';document.getElementById('selected-time').textContent='09月07日 '+row.at;
 ...
 const ruler=document.getElementById('time-ruler');ruler.setAttribute('aria-valuenow',String(i));ruler.setAttribute('aria-valuetext',row.at);document.querySelectorAll('[data-time]').forEach((b,j)=>b.setAttribute('aria-pressed',String(j===i)));paintTrack(i);if(typeof updateMoonPanel==='function')updateMoonPanel(i);
}
function paintTrack(index){const track=document.getElementById('time-track');track.style.transform=`translateX(${-33-index*66}px)`;[...track.children].forEach((b,j)=>{const distance=Math.abs(j-index);b.style.transform=`translateY(${Math.min(12,distance*distance*3)}px)`;b.style.opacity=String(Math.max(.25,1-distance*.22));});}
...
document.querySelectorAll('[data-time]').forEach(b=>b.addEventListener('click',()=>{if(!ignoreTimeClick)updateFacts(Number(b.dataset.time));}));
const ruler=document.getElementById('time-ruler');let timeDrag=null,ignoreTimeClick=false;
ruler.addEventListener('pointerdown',e=>{timeDrag={x:e.clientX,y:e.clientY,index:selectedSlice,axis:null,progress:selectedSlice};ignoreTimeClick=false;});
ruler.addEventListener('pointermove',e=>{if(!timeDrag)return;const dx=e.clientX-timeDrag.x,dy=e.clientY-timeDrag.y;if(!timeDrag.axis&&Math.max(Math.abs(dx),Math.abs(dy))>6)timeDrag.axis=Math.abs(dx)>Math.abs(dy)?'x':'y';if(timeDrag.axis!=='x')return;ruler.setPointerCapture(e.pointerId);ignoreTimeClick=true;const p=Math.max(0,Math.min(24,timeDrag.index-dx/66));timeDrag.progress=p;updateFacts(Math.round(p));paintTrack(p);});
ruler.addEventListener('pointerup',()=>{if(timeDrag?.axis==='x')updateFacts(Math.round(timeDrag.progress));timeDrag=null;});
ruler.addEventListener('pointercancel',()=>{if(timeDrag)updateFacts(timeDrag.index);timeDrag=null;});
ruler.addEventListener('keydown',e=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();updateFacts(e.key==='Home'?0:e.key==='End'?24:Math.max(0,Math.min(24,selectedSlice+(e.key==='ArrowRight'?1:-1))));});
