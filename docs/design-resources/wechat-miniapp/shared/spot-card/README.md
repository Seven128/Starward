...
2026-09-24 生产局部状态：WEAPP Search 和创建/反馈记录共享`components/spot-identity-content.tsx`及`spot-identity-card.scss`的身份内容构图，区域/名称/地址已沿确认的舒适字级方向调整；消费者分别拥有选点动作和审核扩展，媒体授权仍在记录业务责任。当前本机待审样本的候选名称/地址与地图一致，创建记录只读详情会显示冻结的结构化字段；正式反馈详情仍显示原值→新值。记录卡按可见性只读本账户已上传且属于冻结提交的照片，当前本机有图待审样本的列表/详情及单次503→重试恢复已在官方WEAPP日间390×844视口核对。其它视口/主题、审核通过后的照片关系与真机仍待对照，不能据此宣称本预览整体已迁移或候选整套已采用。
