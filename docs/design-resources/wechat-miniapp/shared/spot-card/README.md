...
2026-09-24 生产局部状态：WEAPP Search 和创建/反馈记录共享`components/spot-identity-content.tsx`及`spot-identity-card.scss`的身份内容构图，区域/名称/地址已沿确认的舒适字级方向调整；消费者分别拥有选点动作和审核扩展，不把媒体授权交给视觉组件。当前本机待审样本的候选名称/地址与地图一致，创建记录只读详情会显示冻结的结构化字段；正式反馈详情仍显示原值→新值。实际原图只覆盖390×844日间及无照片记录；记录卡片有图时的授权读取、其它视口与主题仍待对照，不能据此宣称本预览整体已迁移或候选整套已采用。
