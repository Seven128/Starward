...
(() => {
  ...
  window.StarwardObservationTime = {
    mount(root, { days, dayIndex = 7, index = 6, moonSource, onChange = () => {} }) {
      ...
      let presented = index, motionFrame = 0, committedIndex=index; const F=StarwardFluid; let settlement=null;
      ...
      function paint(progress) {
        presented = progress;if(settlement!==null){const next=clamp(Math.round(progress));if(next!==selectedIndex){selectedIndex=next;render();onChange(state(),'preview');}}
      ...
      const movement=F.scalar(index*66,v=>paint(v/66));
      ...
      function setIndex(i, phase) { selectedIndex = clamp(i); render(); if(phase==='commit'){settle(selectedIndex,0,true);}else{onChange(state(),phase);if(phase==='cancel')settle(selectedIndex);} }
      function cancel() { const pending=!!drag||movement.active||selectedIndex!==committedIndex;drag=null;stopMotion();selectedIndex=committedIndex;render();settle(committedIndex);if(pending)onChange(state(),'cancel'); }
      function chooseDay(i) {
        ...
        committedIndex=selectedIndex;render();movement.set(selectedIndex*66);onChange(state(),'commit');
      }
      ...
      listen(ruler, 'pointerdown', e => {
        ...
        if (drag) cancel();
        const interrupted = movement.active;
        stopMotion(); suppressClick = interrupted; drag = { id:e.pointerId, x:e.clientX, y:e.clientY, index:selectedIndex, origin:F.unRubber(presented*66,0,clamp(Infinity)*66,32)/66, axis:null, progress:presented, samples:[{v:-e.clientX,t:e.timeStamp}] };
      });
      listen(ruler, 'pointermove', e => {
        ...
        ruler.setPointerCapture(e.pointerId); suppressClick = true; drag.raw=(drag.origin*66-dx);drag.progress=F.rubber(drag.raw,0,clamp(Infinity)*66,32)/66;drag.samples.push({v:-e.clientX,t:e.timeStamp});drag.samples=drag.samples.slice(-24);
        setIndex(Math.round(drag.progress), 'preview'); movement.set(drag.progress*66);root.dataset.motion='dragging';
      });
      listen(ruler, 'pointerup', e => { const ended=drag;drag=null;root.dataset.motion='idle';if(ended?.axis==='x'){const v=reducedMotion.matches?0:F.velocity(ended.samples,e.timeStamp);const target=F.projected(ended.progress,v/66,0,clamp(Infinity),150);settle(target,v*F.resistance(ended.raw,0,clamp(Infinity)*66,32),true);}else if(ended)cancel(); });
      listen(ruler, 'pointercancel', () => { suppressClick=true; cancel(); });
      listen(ruler, 'lostpointercapture', () => { if (drag) { suppressClick=true; cancel(); } });
      listen(window,'pointerup',()=>{if(drag&&drag.axis!=='x')cancel();});
      listen(window,'pointerdown',e=>{if(!e.isPrimary&&drag){suppressClick=true;cancel();}}, {capture:true});
      ...
      listen(document, 'visibilitychange', () => { if (document.hidden) { cancel(); stopMotion(); movement.set(committedIndex*66); } });
      listen(reducedMotion, 'change', () => { if (reducedMotion.matches) { cancel(); movement.finish(); } });
      render();movement.set(selectedIndex*66);
      return { cancel() { cancel(); stopMotion(); movement.set(committedIndex*66); }, getState: state, destroy() { cancel(); stopMotion(); listeners.abort(); calendar.remove(); } };
    }
  };
})();
