...
2026-09-15 外部能力方案采用补充：此包继续控制弹窗构图、浏览/单选、动画、返回和滚动；原六条年度示例及截图中的 IAU/IMO、极大日期不控制 GMN 常年资料含义。GMN 的“参考／常年参考日（UTC）／监测参考期”、缺方向/流量及独立日期查询由[当前共享产品契约](../../../../../project_context/areas/main/screen-contracts/wechat-miniapp/shared-state-and-recovery.md#shared-astronomical-event-modal)控制，生产内容入口为 `apps/wechat-miniapp/src/content/event/event-model.ts` 和共享modal。当前HTML仍是旧年度示例的可编辑交互资源，尚未反映新数据语义；不将旧截图称为新版内容预览。没有新增构图或重新生成素材。
...
2026-09-25 生产状态补证：共享事件弹层的当地投影 `UNAVAILABLE` 现保留事件身份、日期与来源，以公共非空结果提示表达不可用；刷新失败转为可重试错误。目录失败或部分资料不会同时显示真实空目录卡，只有确认的新鲜空结果使用公共空态。官方WEAPP模拟器已在当前仓库构建下复测Map浏览的17条目录、详情与返回，以只读有效响应注入核对当地条件不可用画面，并抽查计划单选的目录、详情、返回和取消；计划实际写回、真机与正式供应商异常仍按任务证据单列。此处仅记录生产迁移和验证范围，不改变本资源采用状态或旧原型内容。
