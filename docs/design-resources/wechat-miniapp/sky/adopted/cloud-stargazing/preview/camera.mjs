 ...
 */
export function createSkyViewBasis(headingDeg, betaDeg, gammaDeg) {
...
function validBasis(value) {
    ...
    if (!validVector(right) ||
        !validVector(up) ||
        !validVector(forward))
        return false;
    const vectors = [right, up, forward];
    for (const current of vectors) {
        if (Math.abs(Math.hypot(...current) - 1) > BASIS_TOLERANCE)
            return false;
    }
    const rightCrossUp = cross(right, up);
    return (Math.abs(dot(right, up)) <= BASIS_TOLERANCE &&
        Math.abs(dot(right, forward)) <= BASIS_TOLERANCE &&
        Math.abs(dot(up, forward)) <= BASIS_TOLERANCE &&
        rightCrossUp.every((component, index) => Math.abs(component + forward[index]) <= BASIS_TOLERANCE));
}
/**
 ...
 */
export function projectSkyDirection(azimuthDeg, altitudeDeg, basis, width, height, verticalFovDeg) {
    if (!finite(azimuthDeg) ||
        !finite(altitudeDeg) ||
        altitudeDeg < -90 ||
        altitudeDeg > 90 ||
        !validBasis(basis) ||
    ...
    if (!finite(focalLength) || focalLength <= 0)
        return null;
 ...
 */
export function calibrateSkyHeadingOffset(compassHeadingDeg, alphaDeg) {
    if (!finite(compassHeadingDeg) || !finite(alphaDeg))
        return null;
...
export function resolveSkyHeading(offsetDeg, alphaDeg) {
    if (!finite(offsetDeg) || !finite(alphaDeg))
        return null;
