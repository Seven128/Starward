// Generated from the existing settings gesture owner by build-mode.cjs; do not hand-edit.
export const DISPLAY_MODES = ["DAY", "NIGHT", "OBSERVATION"];
export const DISPLAY_MODE_LABEL = {
    DAY: "日间", NIGHT: "夜间", OBSERVATION: "观测红光",
};
export function tappedMode(current, target) {
    return current === target
        ? DISPLAY_MODES[(DISPLAY_MODES.indexOf(current) + 1) % DISPLAY_MODES.length]
        : target;
}
export function moveModeDrag(drag, x, y, at) {
    const dx = x - drag.x;
    const dy = y - drag.y;
    ...
    if (drag.axis === "horizontal" && drag.step > 0) {
        ...
        if (elapsed > 0)
            drag.velocity = (x - drag.lastX) / elapsed;
    }
    drag.lastX = x;
    drag.lastAt = at;
    return drag;
}
export function releasedMode(drag, committed, at) {
    if (drag.axis !== "horizontal" || !(drag.step > 0))
        return committed;
