...
async function buildDesign({ root, page, direction, width = 390, height = 844, mode = 'day', state, fonts, icons, mapImageHash, fixture }) {
  ...
  root.setPluginData('direction', String(direction)); root.setPluginData('round', '1');
  ...
  function rect(parent, name, x, y, w, h, fill, radius = 0) {
    const n = figma.createRectangle(); parent.appendChild(n); n.name = name; n.resize(w, h); n.fills = paint(fill); n.cornerRadius = radius;
    if ('layoutMode' in parent && parent.layoutMode !== 'NONE') n.layoutPositioning = 'ABSOLUTE';
    return position(n, x, y);
  }
  ...
  async function button(parent, { key, label, glyph, w = 44, h = 44, owner = owners.map, fill, color = C.ink, radius = 8, size = 14, line = 20, visible = 32, selected = false }) {
    ...
    if (key === 'search') { body.primaryAxisAlignItems = 'MIN'; body.paddingLeft = 14; body.itemSpacing = 9; }
  ...
  async function mapPage() {
    ...
    const doc = stack(clip, 'Single retained spot document', width); doc.counterAxisAlignItems = 'CENTER';
  ...
  async function myEntry(parent, { key, title, subtitle, glyph, color = C.secondary, soft = C.subtle, w, compact = false, side, foot, footMeta }) {
    ...
    if (footMeta) await text(col, footMeta, col.width, 12, 18, 'regular', C.secondary);
  ...
  async function myPage() {
    ...
    const account = row(content, 'Compact account identity', content.width, 66, 10);
    const av = frame(account, 'Current account avatar', 34, 34, C.subtle, direction === 1 ? 8 : 17); position(icon(av, 'user-round', 20, C.secondary), 7, 7);
    ...
    if (direction === 0) {
      const plan = stack(utilities, 'Entry/plan', uw, 4); plan.paddingTop = 12; plan.paddingBottom = 12; tag(plan, 'plan', owners.my, '今晚计划');
      const heading = row(plan, 'Plan label and date', uw, 28, 10);
      const tile = frame(heading, 'Plan semantic tile', 28, 28, C.greenSoft, 9); position(icon(tile, 'calendar-days', 17, C.green), 5.5, 5.5);
      await text(heading, '今晚计划', uw - 174, 14, 20, 'medium');
      const date = await text(heading, f.plan.date, 102, 12, 18, 'regular', C.secondary); date.textAlignHorizontal = 'RIGHT'; icon(heading, 'chevron-right', 14, C.muted);
      const details = stack(plan, 'Plan destination and preparation', uw, 2); details.paddingLeft = 38;
      await text(details, f.plan.spotName, uw - 38, 14, 20, 'regular'); await text(details, f.plan.detail, uw - 38, 12, 18, 'regular', C.secondary);
    ...
    frame(content, 'Utility routine separation', content.width, 18);
