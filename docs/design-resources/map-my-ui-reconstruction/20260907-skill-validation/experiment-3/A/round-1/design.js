...
async function buildDesign({ root, page, direction, width, height, mode, state, fonts, icons, mapImageHash, fixture }) {
  ...
  function icon(p,key,size=18,color=C.ink) {
    ...
    if (!iconCache[cacheKey]) {
      const sourceKey=key==='telescope' ? 'telescope' : (icons[key+'-day'] ? key+'-day' : key);
      ...
      const art=figma.createNodeFromSvg(svg); comp.appendChild(art); art.rescale(size/art.width); art.name=sourceKey;
  ...
  if(page==='map') {
    ...
    const viewportHeight=direction===2?328:330;
    const viewport=at(box(panel,'medium 文档裁切；large 唯一纵向滚动',width,viewportHeight),0,44); viewport.clipsContent=true;
    ...
    let time;
    if(direction===1) {
      const astronomyHeader=row(astro,'天文标题与所选时间',w,12,0,null,0,44);
      text(astronomyHeader,'天文信息',80,16,23,'medium');
      time=control(box(astronomyHeader,'所选日期与时间',w-92,44),'time',O.time,'adjustable');
      at(text(time,fixture.date+' · '+fixture.time,w-92,14,21,'medium',C.sky),0,11);
      text(astro,'时间切片暂无数据',w,12,18,'regular',C.sub);
    } else {
      text(astro,'天文信息',w,16,23,'medium');
      time=control(stack(astro,'所选日期与时间',w,5),'time',O.time,'adjustable');
    ...
    const laneY=44+viewportHeight;
    const lane=at(box(panel,'底部独立操作区',width,430-laneY,C.white),0,laneY);
    ...
    if(direction===0) {
      ...
      const plan=control(row(utility,'今晚计划',w,10,0,null,0,96),'plan',O.my); tile(plan,'calendar-days',C.sky,C.skySoft); const pc=stack(plan,'计划信息',w-64,4); const pr=row(pc,'日期行',w-64,6); text(pr,'今晚计划',80,14,21,'medium'); text(pr,planDate,w-150,12,18,'regular',C.sub); text(pc,fixture.plan.spotName,w-64,15,22,'regular'); text(pc,fixture.plan.detail,w-64,12,18,'regular',C.sub); chevron(plan);
      ...
      const plan=control(row(shared,'今晚计划',w,14,0,null,0,90),'plan',O.my);
      const label=stack(plan,'索引列',104,8); tile(label,'calendar-days',C.sky,C.skySoft); text(label,'今晚计划',104,14,21,'medium');
      const data=stack(plan,'计划正文',w-148,3); text(data,planDate,w-148,12,18,'regular',C.sub); text(data,fixture.plan.spotName,w-148,15,22,'regular'); text(data,fixture.plan.detail,w-148,12,18,'regular',C.sub); chevron(plan); line(shared,w);
      const contribution=control(row(shared,'现场反馈与纠错',w,14,0,null,0,76),'contribution',O.my); const label2=stack(contribution,'索引列',104,6); tile(label2,'clipboard-check',C.green,C.greenSoft); text(label2,'现场反馈与纠错',104,14,21,'medium'); const status=stack(contribution,'真实审核状态',w-148,4); text(status,fixture.account.drafts+' 条草稿 · '+fixture.account.pending+' 条待审核',w-148,12,18,'regular',C.sub); chevron(contribution); line(shared,w);
      box(main,'内容组间距',w,20);
      [['profile-links','主页链接',fixture.account.profileLinks+' 条已保存','link',C.sky,C.skySoft],['import','内容导入','导入自己的帖子并提交审核','file-up',C.gold,C.goldSoft]].forEach(([k,l,s,g,c,b])=>{ const n=control(row(main,l,w,14,0,null,0,70),k,O.my); const lead=stack(n,'索引列',104,6); tile(lead,g,c,b); text(lead,l,104,14,21,'medium'); text(n,s,w-148,12,18,'regular',C.sub); chevron(n); line(main,w); });
    } else {
      const utilities=stack(main,'计划与反馈共享表面',w,0,12,C.neutral,10); const iw=w-24;
      const plan=control(row(utilities,'今晚计划',iw,10,0,null,0,82),'plan',O.my); const rail=box(plan,'计划文字组',iw-26,1); rail.layoutMode='VERTICAL'; rail.primaryAxisSizingMode='AUTO'; rail.counterAxisSizingMode='FIXED'; rail.itemSpacing=4; const pl=row(rail,'计划标签',iw-26,7); icon(pl,'calendar-days',16,C.sky); text(pl,'今晚计划 · '+planDate,iw-49,12,18,'medium',C.sub); text(rail,fixture.plan.spotName,iw-26,15,22,'regular'); text(rail,fixture.plan.detail,iw-26,12,18,'regular',C.sub); chevron(plan);
      line(utilities,iw);
