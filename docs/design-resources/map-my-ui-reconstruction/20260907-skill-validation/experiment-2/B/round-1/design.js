...
async function buildDesign({root, page, direction, width, height, mode, state, fonts, icons, mapImageHash, fixture}) {
  ...
  function textNode(parent,name,content,size=14,line=20,color=P.ink,medium=false,w=null) {
    const n=figma.createText(); n.name=name; n.fontName=medium==='bold'?fonts.bold:(medium?fonts.medium:fonts.regular);
  ...
  function icon(parent,name,color=accent,size=18) {
    const source=name==='telescope'?(icons.telescope||icons['telescope-day']):(icons[name+'-'+mode]||icons[name]);
  ...
  function shell(){
    ...
    box(cap,'Capsule divider',1,14,P.line,0,44,8);
  ...
  if(page==='map'){
    ...
    const panelY=d===2?448:430, navY=H-90, actionY=navY-48;
    ...
    const identity=box(doc,'Spot identity',W,d===2?34:52);
    if(d===0){box(identity,'Identity accent',2,37,accent,1,16,2);atText(identity,'Spot name',fixture.spot.name,27,0,19,27,P.ink,'bold',W-43);atText(identity,'Region',fixture.spot.region,27,29,12,18,P.secondary);}
    if(d===1){atText(identity,'Region',fixture.spot.region,16,0,12,18,P.secondary);atText(identity,'Spot name',fixture.spot.name,16,20,20,28,P.ink,'bold',W-32);}
    if(d===2){atText(identity,'Spot name',fixture.spot.name,16,0,19,27,P.ink,'bold',W-136);atText(identity,'Region',fixture.spot.region,W-93,7,12,18,P.secondary);}
    ...
    const evidenceWidth=Math.floor((W-36)/3);
    for(let i=0;i<3;i++){const e=control(links,keys[i],labels[i],{w:evidenceWidth,h:44,size:12,color:P.secondary,owner:OWN.panel});e.x=16+i*(evidenceWidth+2);}
    ...
    const head=box(content,'Compact account header',W-32,66);
    const av=box(head,'Account avatar',36,36,tint,12,0,0);const ai=icon(av,'user-round',accent,21);ai.x=7.5;ai.y=7.5;
    atText(head,'Page identity',fixture.account.title,48,-2,20,28,P.ink,'bold',W-132);
    ...
    if(d===0){
      ...
      const plan=box(utilities,'Tonight plan',W-56,80);StarwardFigma.control(plan,'plan',OWN.my);
      ...
      line(utilities,W-56);
      rowEntry(utilities,'contribution','现场反馈与纠错',fixture.account.drafts+' 条草稿 · '+fixture.account.pending+' 条待审核','message-square-warning',{w:W-56,h:60,small:true});
    ...
    if(d===1){
      ...
      const plan=box(utility,'Tonight object / plan entry',W-32,106,P.greenSoft,12);StarwardFigma.control(plan,'plan',OWN.my);
      atText(plan,'Plan label','今晚计划',14,12,12,18,P.green,true);
      atText(plan,'Date',fixture.plan.date,W-32-106,12,12,18,P.green);
      atText(plan,'Plan destination',fixture.plan.spotName,14,38,17,25,P.ink,'bold',W-80);
      atText(plan,'Preparation detail',fixture.plan.detail,14,70,13,20,P.secondary);
      const arrow=icon(plan,'chevron-right',P.green,18);arrow.x=W-68;arrow.y=69;
      const c=rowEntry(utility,'contribution','现场反馈与纠错',fixture.account.drafts+' 条草稿 · '+fixture.account.pending+' 条待审核','message-square-warning',{h:67});
      line(content,W-32);
    ...
    if(d===2){
      const utility=box(content,'Planning and contribution / shared desk',W-32,136,P.soft,10);
      const col=Math.floor((W-68)/2);
      const plan=box(utility,'Compact planning record',col,112,null,0,12,12);StarwardFigma.control(plan,'plan',OWN.my);
      atText(plan,'Plan label','今晚计划',0,0,13,20,P.ink,true);
      atText(plan,'Plan object',fixture.plan.spotName,0,31,14,21,P.ink,false,col);
      atText(plan,'Plan date',fixture.plan.date,0,58,12,18,P.secondary);
      atText(plan,'Plan detail',fixture.plan.detail,0,81,12,18,P.secondary,false,col);
      const ar=icon(plan,'chevron-right',P.secondary,15);ar.x=col-15;ar.y=3;
      box(utility,'Desk divider',1,102,P.line,0,col+18,17);
      const pending=box(utility,'Contribution status / one entry',col,112,null,0,col+24,12);StarwardFigma.control(pending,'contribution',OWN.my);
      atText(pending,'Contribution label','现场反馈与纠错',0,0,13,20,P.ink,true,col-16);
      atText(pending,'Draft count',fixture.account.drafts+' 条草稿',0,34,14,21,accent);
      atText(pending,'Pending count',fixture.account.pending+' 条待审核',0,62,14,21,accent);
      const ar2=icon(pending,'chevron-right',P.secondary,15);ar2.x=col-15;ar2.y=3;
  ...
  nav();
  root.setPluginData('visualReview','Round 1 follows actual six round-0 PNG review; awaiting this revision export.');
