...
async function buildDesign({root,page,direction,width=390,height=844,mode='day',state,fonts,icons,mapImageHash,fixture}) {
  ...
  if(page==='map'){
    ...
    if(D===0){
      ...
      const ch=row(contrib,'Contribution icon and title',colW-24,24,7);ico(ch,'message-square-warning',17,C.sky);text(ch,'Contribution heading','现场反馈与纠错',colW-48,15,22,'medium');
      text(contrib,'Draft state',fixture.account.drafts+' 条草稿',colW-24,13,20,'regular',C.secondary);text(contrib,'Moderation state',fixture.account.pending+' 条待审核',colW-24,13,20,'regular',C.secondary);
