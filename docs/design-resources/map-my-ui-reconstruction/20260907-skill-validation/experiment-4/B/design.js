...
async function buildDesign({root,page,direction,width=390,height=844,mode='day',state,fonts,icons,mapImageHash,fixture}) {
  ...
  function ico(p,key,size=18,color=C.ink){
    ...
    if(!iconComponents[id]){
      ...
      // Mode assets contain authored colors; keep their legitimate artwork intact.
      iconComponents[id]=c;
    }
  ...
  if(page==='map'){
    ...
    rule(doc,cw);
    const astro=stack(doc,'Astronomy section / remains below medium crop',cw,10);astro.paddingTop=15;
    ...
    const avatar=ico(header,'neutral-avatar',34);avatar.name='Current account avatar';
    ...
    function utility(p,key,title,icon,details,ww,style='plain'){
      ...
      const inner=ww-f.paddingLeft-f.paddingRight;const head=row(f,'Utility header',inner,24,8);const i=ico(head,icon,17,key==='plan'?C.green:C.sky);text(head,'Utility title',title,inner-44,15,22,'medium');ico(head,'chevron-right',14,C.secondary);
    ...
    if(D===0){
      ...
      utility(group,'plan','今晚计划','route',[[fixture.plan.date,'Plan date'],[fixture.plan.spotName,'Plan place'],[fixture.plan.detail,'Plan detail']],cw-24);
      ...
      const group=stack(content,'Plan and contribution / field notes group',cw,0);
      const pl=utility(group,'plan','今晚计划','route',[[fixture.plan.date,'Plan date'],[fixture.plan.spotName,'Plan place'],[fixture.plan.detail,'Plan detail']],cw,'tonal');pl.fills=fill(C.subtle);pl.cornerRadius=4;
      const contrib=control(row(group,'Contribution status entry',cw,77,10,12),'contribution',own.my);const ic=ico(contrib,'message-square-warning',17,C.sky);
      const cs=stack(contrib,'Contribution text',cw-75,4);text(cs,'Contribution heading','现场反馈与纠错',cw-75,15,22,'medium');text(cs,'Contribution state',fixture.account.drafts+' 条草稿 · '+fixture.account.pending+' 条待审核',cw-75,13,20,'regular',C.secondary);ico(contrib,'chevron-right',14,C.secondary);
      rule(content,cw);frame(content,'Routine entries spacing',0,0,cw,8);
      routine(content,'profile-links','主页链接',fixture.account.profileLinks+' 条已保存','link',C.sky,C.skySoft);rule(content,cw);
      routine(content,'import','内容导入','导入自己的帖子并提交审核','file-up',C.gold,C.goldSoft);
    }
