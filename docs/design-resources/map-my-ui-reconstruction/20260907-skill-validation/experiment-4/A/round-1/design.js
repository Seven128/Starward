...
async function buildDesign({root,page,direction,width,height,mode,state,fonts,icons,mapImageHash,fixture}) {
  ...
  function icon(parent,key,size=18,color=C.ink,source='mode') {
    const ck=key+'/'+size+'/'+color;
    if(!iconCache.has(ck)) {
      const svg=source==='linear'?icons[key]:(icons[key+'-day'] || icons[key]);
      ...
      const imported=F.icon(m,svg.replace(/currentColor/g,color),{size,name:key});
      // Shared single-color line glyphs may arrive with pre-resolved colors.
      for(const v of imported.findAll(n=>('strokes' in n)||('fills' in n))) {
        if(Array.isArray(v.strokes)&&v.strokes.length)v.strokes=v.strokes.map(p=>p.type==='SOLID'?{...p,color:rgb(color)}:p);
        if(Array.isArray(v.fills)&&v.fills.length)v.fills=v.fills.map(p=>p.type==='SOLID'?{...p,color:rgb(color)}:p);
      }
      iconCache.set(ck,m);
    }
  ...
  function control(parent,key,w,h,label,glyph,{bg=null,color=C.ink,owner=O.panel,vertical=false,r=8,size=14}={}) {
    ...
    if(key==='search'){content.primaryAxisAlignItems='MIN';content.paddingLeft=14;}
    if(glyph)icon(content,glyph,18,color,glyph==='telescope'?'linear':'mode');
  ...
  if(page==='map') {
    ...
    // Crop on the complete metrics block boundary, keeping all later nodes retained.
    // A shorter identity in direction 2 must not expose half the next text line.
    viewport.resize(width,Math.min(viewport.height,metrics.y+metrics.height));
    ...
    const avatarTile=frame(account,'Neutral account icon surface',34,34,0,9,C.soft,10);
    const avatar=icon(avatarTile,'user-round',23,C.secondary,'linear');avatar.x=5.5;avatar.y=5.5;
    ...
    function entry(key,label,glyph,color,bg,h,build) {
      ...
      atText(n,label,x,9,uw-x-24,14,20,C.ink,'medium');const arrow=icon(n,'chevron-right',16,C.secondary);arrow.x=uw-18;arrow.y=15;
      build(n,x,uw-x-24);return n;
    }
    if(direction===1) {
      utility.layoutMode='HORIZONTAL';utility.resize(inner,128);utility.primaryAxisSizingMode='FIXED';utility.counterAxisSizingMode='FIXED';utility.itemSpacing=20;
      const cw=(uw-20)/2;
      for(const [key,title,glyph,color,bg] of [['plan','今晚计划','calendar-days',C.green,C.greenSoft],['contribution','现场反馈与纠错','message-square-warning',C.gold,C.goldSoft]]) {
        const n=frame(utility,title,cw,112);F.control(n,key,O.my);n.setPluginData('accessibleName',title);
        const tile=frame(n,'Role icon tile',24,24,0,2,bg,7);const ic=icon(tile,glyph,16,color);ic.x=4;ic.y=4;
        atText(n,title,32,3,cw-32,14,20,C.ink,'medium');
        if(key==='plan') {
          atText(n,fixture.plan.date,0,36,cw,12,18,C.secondary);
          atText(n,fixture.plan.spotName,0,60,cw,14,21,C.ink);
          atText(n,fixture.plan.detail,0,84,cw,12,18,C.secondary);
        } else {
          atText(n,fixture.account.drafts+' 条草稿',0,36,cw,14,21,C.ink);
          atText(n,fixture.account.pending+' 条待审核',0,62,cw,14,21,C.secondary);
        }
      }
    } else {
    entry('plan','今晚计划','calendar-days',C.green,C.greenSoft,88,(n,x,w)=>{
      atText(n,fixture.plan.date,x,37,94,12,18,C.secondary);
      atText(n,fixture.plan.spotName,x+98,35,w-98,14,21,C.ink);
      atText(n,fixture.plan.detail,x,62,w,12,18,C.secondary);
    });
    rule(utility,uw);
    entry('contribution','现场反馈与纠错','message-square-warning',C.gold,C.goldSoft,70,(n,x,w)=>{
      ...
      else atText(n,fixture.account.drafts+' 条草稿  ·  '+fixture.account.pending+' 条待审核',x,37,w,14,21,C.secondary);
    });
    }
    ...
    function routine(key,label,summary,glyph,color,bg) {
      const n=frame(list,label,inner,68);F.control(n,key,O.my);n.setPluginData('accessibleName',label);n.setPluginData('interactionState','Opens existing recoverable child; no fabricated counters');
      const tile=frame(n,'Role icon tile',28,28,0,18,bg,8);const ic=icon(tile,glyph,17,color);ic.x=5.5;ic.y=5.5;
      atText(n,label,40,10,inner-64,14,20,C.ink,'medium');atText(n,summary,40,35,inner-64,14,21,C.secondary);
  ...
  navigation();
  root.setPluginData('round','round-1; revised after actual six-image round-0 review; awaiting new export');
  return root;
}
