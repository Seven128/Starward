/* Experiment 1 / B. Native Figma Plugin API. Source and fixture notes: rationale.md.
...
async function buildDesign({ root, page, direction, width = 390, height = 844, mode = 'day', state, fonts, icons, mapImageHash, fixture = {} }) {
  ...
  root.setPluginData('designRound','0');
  ...
  const shell=()=>{
    ...
    at(text(s,'System status','•••  ▰',64,12,18,C.ink),width-82,14);
    ...
    at(text(cap,'WeChat menu','•••',28,16,23,C.ink,true),12,3);
    rect(cap,'Capsule divider',47,8,1,14,C.line);at(text(cap,'WeChat close','◉',20,16,23),57,3);
  ...
  if(page==='my'){
    ...
    if(direction===0){
      ...
      const plan=rect(utility,'Tonight plan',0,0,W,138,null);
      at(text(plan,'Date month','09 月',64,14,21,C.secondary),0,9);at(text(plan,'Date day','07',64,20,28,C.ink,true),0,32);
      const cp=stack(plan,'Plan information',W-92,6);at(cp,84,6);
      text(cp,'Title','今晚计划',W-92,16,23,C.ink,true);text(cp,'Date','2026-09-07',W-92,12,18,C.secondary);text(cp,'Place','深圳市天文台',W-92,18,25,C.ink,true);text(cp,'Preparation','地点与出发准备',W-92,14,21,C.secondary);mark(plan,'plan',OWNER.my);
      line(utility,W);
      const con=rect(utility,'Contribution',0,0,W,92,null);at(text(con,'Label','反馈',64,14,21,C.secondary),0,17);
    ...
    if(direction===0){
      const access=stack(basic,'Access facts',W,5);text(access,'Open hours','开放时间  暂无数据',W,15,22);text(access,'Night access','夜间进入条件待核实',W,14,21,C.secondary);
      const facilities=fixed(basic,'Shared facility line',W,26);facilities.primaryAxisAlignItems='MIN';facilities.itemSpacing=16;text(facilities,'Parking','停车  暂无数据',(W-16)/2,14,21,C.secondary);text(facilities,'Toilet','厕所  暂无数据',(W-16)/2,14,21,C.secondary);
      line(basic,W);
    } else if(direction===1){
      ...
      const pair=stack(facts,'Facility pair',W-24,16,true);miniFact(pair,'停车','暂无数据',(W-40)/2);miniFact(pair,'厕所','暂无数据',(W-40)/2);
      line(facts,W-24);text(facts,'Open hours','开放时间  暂无数据',W-24,15,22);
      ...
      const facts=stack(access,'Access values',W-70,5);text(facts,'Open hours','开放时间  暂无数据',W-70,15,22);text(facts,'Night access','夜间进入条件待核实',W-70,14,21,C.secondary);
      line(basic,W);
      const facilities=stack(basic,'Facility label band',W,14,true);text(facilities,'Label','设施',56,14,21,C.secondary);text(facilities,'Values','停车、厕所暂无数据',W-70,15,22);
    }
    ...
    line(doc,W);
    const astro=stack(doc,'Astronomy / continuous document',W,10);text(astro,'Astronomy heading','天文信息',W,16,23,C.ink,true);
    const ruler=mark(rect(astro,'Curved time ruler / real-slice fixture',0,0,W,70,null),'time',OWNER.time);
    at(text(ruler,'Selected date and time','09-07 21:00',130,18,25,C.blue,true),W/2-65,0);
    for(let i=-4;i<=4;i++){
      const d=Math.abs(i);const tickH=i===0?20:(d%2===0?12:7);const tickY=31+d*1.5;
      rect(ruler,`Real slice tick ${i}`,W/2+i*34,tickY,i===0?2:1,tickH,i===0?C.blue:C.strong);
    }
    at(text(ruler,'Earlier tick','19:00',46,12,18,C.secondary),W/2-151,51);at(text(ruler,'Later tick','23:00',46,12,18,C.secondary),W/2+109,51);
