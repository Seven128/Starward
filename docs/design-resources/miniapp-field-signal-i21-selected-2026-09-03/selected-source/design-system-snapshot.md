...
移动表单与组件衔接进一步吸收开源组件库的可迁移逻辑而不复制品牌表面：Ant Design Mobile 的 Form/List/Selector/ImageUploader 以原子字段、就地反馈、共享行节奏和单一提交减少重复容器；Ant Design proximity 以一致近邻表达归属；TDesign Mini Program、Vant Weapp、Taroify 与 NutUI Taro 共同证明紧凑来自 token 化的 label/value 轴、稳定目标、局部状态和少量 section gap，而非缩小可读性。当前 Taro `4.2.1` + React `18.3.1` 实现后续以 MIT `@taroify/core@1.0.6` 作为 preferred generic component substrate，并通过单一 Starward adapter/`ConfigProvider` 投射本节精确 tokens；Ant Design Mobile只作规范参考，TDesign native component只作经验证的隔离 fallback，NutUI当前beta不作首选。来源：<https://mobile.ant.design/zh/components/form/>、<https://mobile.ant.design/zh/components/list/>、<https://mobile.ant.design/zh/components/selector/>、<https://mobile.ant.design/zh/components/image-uploader/>、<https://mobile.ant.design/zh/guide/faq/>、<https://ant.design/docs/spec/proximity/>、<https://github.com/Tencent/tdesign-miniprogram>、<https://docs.taro.zone/en/docs/hybrid>、<https://github.com/taroify/taroify>、<https://taroify.com/components/form/>、<https://taroify.com/components/config-provider/>、<https://youzan.github.io/vant-weapp/>、<https://nutui.jd.com/taro/react/2x/>。
...
- Preferred mapping：Primary navigation=`Taroify Tabbar`；Search=`Search + Cell.Group/Cell + Checkbox.Group`；spot panel=`FloatingPanel(anchors, contentDraggable=false)`；section/action rails=`Sidebar + Button.Group/Button`；layer sheet=`Popup(bottom)`；Settings/My=`Radio.Group + Cell.Group/Cell + Switch`；Contribution=`Form + Form.Item/Form.Feedback + Field/Input/Textarea + Radio.Group/Checkbox.Group + DatetimePicker/Popup + Uploader/Progress + Button`；短暂异步反馈仅在 `notification-feedback` 需要时用 `Toast`。所有 glyph 仍通过现有 `SemanticIcon`，不得安装第二 icon family。
- Specialized mapping：Curved Time Ruler 继续使用 Taro enhanced horizontal `ScrollView`，因为 generic Slider/Picker 不具备 real-slice curved scrub语义；三态 display track只复用`Radio.Group`可访问性，不复用 binary Switch 外观；Search fixed-frame transition、panel same-document/media phase、handle-only hot region、panel/layer互斥、upload transport/idempotency等均由 Starward owner 包裹。
- `FloatingPanel`只有在真实 WEAPP 证明三 anchors、唯一滚动、nav-safe height、可中断 retarget以及仅`104×40rpx`header热区均成立时才进入 production；失败则在同一 adapter 下回退 Taro `ScrollView` + 当前 panel coordinator，不 fork library、不扩大热区、不改变设计。所有 library imports按需/tree-shaken，version/license/lock/bundle delta与day/night/observation主题投射必须在实现阶段复核。
- 禁止并行引入两套通用 UI suite、复制 library brand defaults、让 library form store成为第二 draft/state truth、使用 runtime CDN/remote asset、或为了声称复用而用语义不等价组件。Taroify升级/替换只能发生在 adapter 下方，不得改变本系统的精确尺寸、层级、边距、语义与产品状态。
...
- Production composition默认使用第4.5节的 Taroify Form/Field/Cell/choice/picker/uploader/progress/button primitives并覆盖为本系统精确token；这不是在viewport展示组件库说明，也不把draft、transport或业务validation交给library。
