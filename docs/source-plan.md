...
- **AC china-compliance-readiness**
  ...
  - Given: 准备面向中国用户开放 production。
  - When: 执行发布就绪审查。
  - Then: EXT china-production-legal-readiness 对 ICP、地图、跨境、存储/CDN和个人信息逐项有外部确认；缺任一项不得宣称生产就绪。
...
- **AC test-coverage**
  ...
  - Given: 候选发布涉及跨午夜、坐标转换、路线、方向、离线和天文算法。
  - When: 执行发布前验证。
  - Then: 单元/黄金/供应商契约/双端模拟与实机/弱无网/低电低亮/户外证据齐全，且天文权威差异在明确容差内；模拟器结果不替代 EXT outdoor-device-field-validation。

<a id="quality-release-observability.acceptance.slo-evaluation"></a>
- **AC slo-evaluation**
  - Accepts: REQ performance-slos
  - Given: 代表性生产/预发流量、主流设备、缓存命中/未命中和数据源运行可观测。
  - When: 按 DEC production-slo-measurement 的窗口评估。
  - Then: 十项来源建议指标分别报告分位/分母/版本；路线超时不阻塞首页并准确标缓存/直线降级，未达标不能用平均值掩盖。
