...
await page.setViewportSize({width:1000,height:760});await page.goto(root+'shared/liquid-glass/review.html');await page.locator('#light[data-glass-mode=refractive]').waitFor();await page.screenshot({path:'output/playwright/glass-07-refraction.png'});
await page.getByRole('button',{name:'仅透光（对照）'}).click();await page.screenshot({path:'output/playwright/glass-07-translucent.png'});
await page.getByRole('button',{name:'透光＋折射',exact:true}).click();await page.getByRole('button',{name:'移动背景'}).click();await page.locator('.background').first().evaluate(e=>Promise.all(e.getAnimations().map(a=>a.finished)));await page.screenshot({path:'output/playwright/glass-07-moved.png'});
