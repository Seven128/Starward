...
import { createTestMiniappService } from './R134-baseline-runtime/workers/miniapp-api/src/test-fixtures/create-test-service.ts';
import { InMemoryTestRepository } from './R134-baseline-runtime/workers/miniapp-api/src/test-fixtures/in-memory-repository.ts';
import { DeterministicWeatherTestAdapter } from './R134-baseline-runtime/workers/miniapp-api/src/test-fixtures/deterministic-weather-adapter.ts';
import { createGaiaDr3SkyCatalogProvider } from './R134-baseline-runtime/workers/miniapp-api/src/sky-scene-catalog.ts';
...
writeFileSync(new URL(process.argv[2] ?? './R134-perf-baseline-rerun.json', import.meta.url), JSON.stringify({ environment: { node: process.version, data: 'synthetic spots, deterministic weather, real committed Gaia catalog, in-memory repository/cache; no remote calls' }, rows }, null, 2));
