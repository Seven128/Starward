...
const rows = [];
for (const count of [8]) {
  const spots = Array.from({ length: count }, (_, i) => ({ ...structuredClone(TEST_PUBLISHED_SPOT), spotId: `spot:perf-${i}`, wgs84: { system: "WGS84", latitude: 22.4 + i * 0.1, longitude: 113.5 + i * 0.05 } }));
...
writeFileSync(new URL(process.argv[2] ?? './R134-perf-distinct-locations.json', import.meta.url), JSON.stringify({ environment: { node: process.version, data: 'synthetic spots, deterministic weather, real committed Gaia catalog, in-memory repository/cache; no remote calls' }, rows }, null, 2));
