...
failMutation=true;
await cache.removeScope('user:a');
...
results.push({scenario:'native manifest replacement and deletion both fail during removeScope', operationResolved:true, manifestStillPresent:stillOnDisk, restoredRemovedBody:Boolean(recovered), expected:'surface incomplete local erasure rather than claim successful cleanup'});
writeFileSync(new URL(process.argv[2] ?? './R2-review-repro-current.json',import.meta.url),JSON.stringify(results,null,2));
