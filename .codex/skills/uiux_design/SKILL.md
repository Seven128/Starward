...
8. [Development execution](../../../AGENTS.md#development-execution) and [Project-local Implementation Decisions](../../../AGENTS.md#project-local-implementation-decisions) own the continuous planning, architecture, robustness, maintainability, performance and verification rules. They apply even when this Skill is not loaded. This Skill supplies target-specific interaction methods; use them within the current module and dependency plan.
...
For a WEAPP canvas composed with ordinary Taro controls above it, use the node API selected by the current owning Context: query `{ node, size }`, size the backing store with the current device pixel ratio, obtain its matching context, and create downloaded bitmap images through that canvas node. Do not pair a typed node with legacy `Taro.createCanvasContext`; it leaves the node without a drawing owner. The expanded Sky owner now selects `Canvas type="webgl"` and TWGL; its production integration must preserve same-layer controls and resource recovery. Historical Android observations established that the legacy native canvas covered Sky overlays while the node 2D path preserved their z-order; that history is not proof of the new composed WebGL surface. Keep a usable list/time/recovery path when canvas creation, image decoding or physical orientation is unavailable, and verify the actual phone surface because desktop layering does not prove native composition.
...
Follow AGENTS.md's check selection, milestone timing and completion rules. The list below supplies relevant dimensions, not every possible combination. Before spreading a shared UI change, inspect a composed target-runtime consumer, including inherited styles, native defaults and displaced rendering. Different overrides, renderers or lifecycle owners require representative observations.

For data-driven UI, perform the intended action with a supported input and inspect its resulting content, including relevant missing/failure and recovery states. After switching, cancellation or a late response, verify which result is current. Selected styling and success messages do not establish the consuming surface's result; use controlled inputs when needed to distinguish expected change from legitimate invariance.
